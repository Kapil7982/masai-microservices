...
export class AuthController {
    ...
    public static endPoint: string="https://masai-blocking-calendar.netlify.app";
    ...
    public static async handleOAuthCallback(req: Request, res: Response) {
        ...
        try {
            ...
            // Store tokens in the local storage for future use
            AuthController.tempToken=tokens
            // after getting token redirct it
            const queryString = `?data=${encodeURIComponent(JSON.stringify(tokens))}`;
            res.redirect(`${AuthController.endPoint}${queryString}`);
        } catch (error) {
